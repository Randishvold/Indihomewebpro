					...
					{#each packages.filter((p) => p.category === '1p') as pkg (pkg.id)}
						<div use:fadeIn>
							<div class="card h-full shadow-sm">
								<div class="bg-white border-0 text-center pt-6 px-6">
									<h5 class="fw-bold text-primary text-lg">{pkg.name}</h5>
								...
								<div class="bg-white border-0 pb-6 px-6">
									<a
										href={pkg.wa_link}
										target="_blank"
			...
			{#if filter === 'all' || filter === 'onedynamic'}
				...
				<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
					{#each packages.filter((p) => p.category === 'onedynamic') as pkg (pkg.id)}
						<div use:fadeIn>
							<div class="card h-full shadow-sm border-2 border-accent">
								<div class="bg-accent/10 border-0 text-center pt-6 px-6">
									<div class="inline-block bg-accent text-gray-800 px-3 py-1 rounded text-sm mb-3">PREMIUM</div>
									<h5 class="fw-bold text-primary text-lg">{pkg.name}</h5>
									<div class="price-display">
										<span class="text-3xl fw-bold text-gray-800">Rp {pkg.price}</span>
										<small class="text-muted">/bulan</small>
